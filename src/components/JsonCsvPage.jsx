...
const SAMPLE_JSON = `[
  {"id": 1, "name": "홍길동", "city": "Seoul", "age": 30},
  {"id": 2, "name": "Jane", "city": "New York", "age": 25},
  {"id": 3, "name": "Yuki", "city": "Tokyo", "age": 28}
]`;

const SAMPLE_CSV = `id,name,city,age
1,홍길동,Seoul,30
2,Jane,New York,25
3,Yuki,Tokyo,28`;
...
const JsonCsvPage = () => {
    ...
    const [mode, setMode] = useState('jsonToCsv'); // jsonToCsv | csvToJson
    ...
    const result = useMemo(() => {
        ...
        try {
            if (mode === 'jsonToCsv') {
                ...
                return { text: '\uFEFF' + csv, error: null };
            } else {
                const parsed = Papa.parse(input, { header: true });
                if (parsed.errors.length) return { text: '', error: parsed.errors[0]?.message || 'CSV 파싱 오류' };
                return { text: JSON.stringify(parsed.data, null, 2), error: null };
            }
        } catch (e) {
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (event) => {
            ...
            if (ext === 'csv') setMode('csvToJson');
            else setMode('jsonToCsv');
        };
    ...
    const handleDownload = () => {
        if (!result.text) return;
        const blob = new Blob([result.text], { type: mode === 'jsonToCsv' ? 'text/csv;charset=utf-8;' : 'application/json;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = fileName ? fileName.replace(/\.[^.]+$/, mode === 'jsonToCsv' ? '.csv' : '.json') : (mode === 'jsonToCsv' ? 'converted.csv' : 'converted.json');
        a.click();
        URL.revokeObjectURL(url);
    };

    const loadSample = () => {
        setInput(mode === 'jsonToCsv' ? SAMPLE_JSON : SAMPLE_CSV);
        setFileName(mode === 'jsonToCsv' ? 'sample.json' : 'sample.csv');
    };
        ...
        <div className="w-full h-full p-5 flex flex-col overflow-hidden" style={{ background: '#08101e' }}>
            ...
            <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/[0.06] flex-shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-bold text-slate-100">JSON ↔ CSV</h2>
                        <p className="text-xs text-slate-500">양방향 변환 · PapaParse 기반</p>
                    </div>
                </div>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
                ...
                <div className="lg:col-span-4 flex flex-col h-full min-h-0">
                    <div className="rounded-xl p-5 flex flex-col h-full overflow-y-auto custom-scrollbar" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                        <div className="mb-4">
                            <h3 className="text-xs font-bold text-slate-300 uppercase mb-3 tracking-wider">Mode</h3>
                            <div className="flex gap-2">
                                <button
                                    onClick={() => setMode('jsonToCsv')}
                                    className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors ${mode === 'jsonToCsv' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                >
                                    JSON → CSV
                                </button>
                                <button
                                    onClick={() => setMode('csvToJson')}
                                    className={`px-4 py-2 text-xs font-bold rounded-lg transition-colors ${mode === 'csvToJson' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                >
                                    CSV → JSON
                                </button>
                            </div>
                        ...
                        <div className="mb-6">
                            ...
                            <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-600 rounded-xl cursor-pointer hover:bg-slate-700/30 transition-colors mb-3">
                                ...
                                <span className="mt-2 text-xs text-slate-400">{mode === 'jsonToCsv' ? 'JSON/CSV 파일 업로드' : 'CSV/JSON 파일 업로드'}</span>
                                <input type="file" className="hidden" accept=".json,.jsonl,.csv" onChange={handleFileUpload} />
                            </label>
                            <textarea
                                value={input}
                                onChange={(e) => setInput(e.target.value)}
                                placeholder={mode === 'jsonToCsv' ? 'JSON 배열 또는 객체를 입력하세요...' : 'CSV 텍스트를 입력하세요...'}
